...
# chain_id shape: `YYYYMMDDTHHMMSS-XXXXXX` (UTC date prefix, T, HHMMSS,
# dash, 6 hex chars). Generated by `prompts.chain_id()`:
#   f"{now.strftime('%Y%m%dT%H%M%S')}-{int(now.timestamp()*1000) & 0xFFFFFF:06x}"
# The regex is FULLY anchored (^...$) so a hand-edited or malicious
# evidence_anchor like "20260601T../../etc/passwd" cannot match — path
# traversal defense in depth. Also resolved-path validated in
# `_chain_id_to_trace_path`.
_CHAIN_ID_RE = re.compile(r"^(\d{4})(\d{2})(\d{2})T\d{6}-[a-f0-9]{6}$")
...
def _chain_id_to_trace_path(
    ...
    """Resolve chain_id → trace file path.

    Two-layer defense against path traversal via crafted evidence_anchor:
    (1) fully-anchored regex (`_CHAIN_ID_RE`) rejects anything that isn't
    shaped like a real chain_id; (2) the resolved path is verified to
    live under `traces_base` so even if the regex ever loosens, an
    attacker can't escape the trace tree.

    Returns None when chain_id is malformed or the resolved path would
    escape `traces_base`.
    """
    match = _CHAIN_ID_RE.match(chain_id)
    if match is None:
        return None
    date_dir = f"{match.group(1)}-{match.group(2)}-{match.group(3)}"
    candidate = traces_base / date_dir / f"{chain_id}.jsonl"
    try:
        candidate.resolve().relative_to(traces_base.resolve())
